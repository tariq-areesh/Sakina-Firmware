...
//
// 6. THRESHOLD
//    Lowered to 0.40 to produce more stressed classifications.
//
// =====================================================================
...
// Lowered from 0.5 → model only needs 40% confidence to label as stressed
constexpr float STRESS_THRESHOLD = 0.40f;
...
float   simStressLevel = 0.4f;  // start already in stressed territory (was 0.0)
...
void updateStressLevel(unsigned long now) {
  ...
  if      (simMode == SIM_FORCE_STRESS) simStressLevel = clampFloat(simStressLevel + 0.10f, 0.0f, 1.0f);
    ...
    // Skewed random walk — drifts toward stress more than calm (was -0.08 to +0.08)
    simStressLevel += randFloat(-0.04f, 0.12f);
...
// which the skewed random walk hits ~85% of the time.
...
void prefillBuffers() {
  // Prefill in stressed territory (norm 0.48 ≈ 34.8 °C) so the first
  // inference window already reflects a stressed state.
  // Was: neutralNorm = 0.57f, bvpBuffer = 0.51f
  constexpr float neutralNorm = 0.48f;
  for (int i = 0; i < BVP_BUFFER_SIZE;  i++) bvpBuffer[i]  = 0.53f;
...
void setup() {
  ...
  simStressLevel     = 0.4f;  // start already stressed (was 0.0)
